...
public class JsonReader
{
	...
	private String tokenName(int t){
		if(t == TOKEN_OBJ_OPEN)return "OBJ_OPEN";
		if(t == TOKEN_OBJ_CLOS)return "OBJ_CLOS";
		if(t == TOKEN_ARR_OPEN)return "ARR_OPEN";
		if(t == TOKEN_ARR_CLOS)return "ARR_CLOS";
		if(t == TOKEN_COMA    )return "COMA";
		if(t == TOKEN_COLON   )return "COLON";
		if(t == TOKEN_LITERAL )return "LITERAL";
		if(t == TOKEN_STRING  )return "STRING";
		if(t == TOKEN_START   )return "START";
	...
	private static LexerVal nextToken(InputStream stream) throws Exception{
		...
		while(true)
		{
			...
			if(Config.DEBUG_LEXER)
				System.out.println("l " + state + " c:" + c + " buff: " + buffer);
			...
			case 0:
				...
				if(d == -1)break;
				if(c == '{') return new LexerVal(TOKEN_OBJ_OPEN,"{");
				if(c == '}') return new LexerVal(TOKEN_OBJ_CLOS,"}");
				if(c == '[') return new LexerVal(TOKEN_ARR_OPEN,"[");
				if(c == ']') return new LexerVal(TOKEN_ARR_CLOS,"]");
				if(c == ':') return new LexerVal(TOKEN_COLON,":");
				if(c == ',') return new LexerVal(TOKEN_COMA,",");
			...
			case 20:
				...
				else
				{
					...
					if(buffer.equals("true") || buffer.equals("false") || buffer.equals("null"))
	...
	private ElementRoot _read() throws Exception
	{
		...
		while(true)
		{
			...
			case 0:
				...
				else if(token == TOKEN_COMA || token == TOKEN_COLON || token == TOKEN_ARR_CLOS)
				{
					throw new Exception("Token " + tokenName(token) + " non attendu");
				}
				else
				{
					skipLexer = true;
			...
			case 900:
				{
					...
					if(p instanceof ElementRoot)
					{
						...
						if(current instanceof ElementArray || current instanceof ElementObject)
							states.push(0);
					}
			...
			if(token == TOKEN_END)
			{
				...
				if(elems.size() > 1)
					throw new Exception("La pile contient " + elems.size() + " éléments ! Retour impossible !");
